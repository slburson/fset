...
(defmethod emacs-inspect-partial ((s set) lo hi)
  (if (<= hi (+ lo 32))
    ...
    (let ((mid (floor (+ lo hi) 2))
      ...
      (append (and (> lo2 lo)
		   (list `(:value ,(make-subrange s lo lo2) ,(format nil "** Show entries ~D - ~D **" lo (1- lo2)))
			 '(:newline)))
	      (emacs-inspect-partial s lo2 hi2)
	      (and (< hi2 hi)
		   (list `(:value ,(make-subrange s hi2 hi) ,(format nil "** Show entries ~D - ~D **" hi2 (1- hi)))
			 '(:newline)))))))
...
(defmethod emacs-inspect-partial ((m map) lo hi)
  (if (<= hi (+ lo 16))
    ...
    (let ((mid (floor (+ lo hi) 2))
      ...
      (append (and (> lo2 lo)
		   (list `(:value ,(make-subrange m lo lo2) ,(format nil "** Show entries ~D - ~D **" lo (1- lo2)))
			 '(:newline)))
	      (emacs-inspect-partial m lo2 hi2)
	      (and (< hi2 hi)
		   (list `(:value ,(make-subrange m hi2 hi) ,(format nil "** Show entries ~D - ~D **" hi2 (1- hi)))
			 '(:newline)))))))
...
(defmethod emacs-inspect-partial ((b bag) lo hi)
  (if (<= hi (+ lo 16))
    ...
    (let ((mid (floor (+ lo hi) 2))
      ...
      (append (and (> lo2 lo)
		   (list `(:value ,(make-subrange b lo lo2) ,(format nil "** Show entries ~D - ~D **" lo (1- lo2)))
			 '(:newline)))
	      (emacs-inspect-partial b lo2 hi2)
	      (and (< hi2 hi)
		   (list `(:value ,(make-subrange b hi2 hi) ,(format nil "** Show entries ~D - ~D **" hi2 (1- hi)))
			 '(:newline)))))))

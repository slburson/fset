  ...
  (macrolet ((test (form)
	...
	(let ((wbb (wb-bag))
	  ...
	  (dotimes (j (+ 64 (random 128)))
		...
		(let ((mult (1+ (random 3))))
		  (dotimes (k mult)
		    (setq wbb (with wbb mi))
		    (setq chb (with chb mi)))))
	      (test (= (multiplicity chb mi) (multiplicity wbb mi)))
	      (test (verify chb))
		...
		(test (equal? chb (convert 'ch-bag (convert 'list chb))))
		(test (equal? chb (convert 'ch-bag (convert 'vector chb))))
		(test (equal? chb (convert 'ch-bag (convert 'seq chb))))
		;; Test alist conversions
		(test (equal? chb (convert 'ch-bag (convert 'alist chb) :from-type 'alist))))))

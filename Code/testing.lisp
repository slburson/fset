...
(defun Test-Map-Operations (i a-set)
  ...
  (let ((fm0 (empty-map))
    ...
    (unless (equal? fm0 (convert 'map (convert 'alist fm0) :input-sorted? t))
      (error "Map conversion from sorted list failed (fm0) on iteration ~D" i))
    (unless (equal? fm0 (convert 'map (convert 'seq m0)))
      (error "Map conversion from seq failed (fm0) on iteration ~D" i))
    (unless (equal? fm0 (convert 'map (sort (convert 'seq m0) #'less-than? :key #'car) :input-sorted? t))
      (error "Map conversion from sorted seq failed (fm0) on iteration ~D" i))
    (unless (equal? fm0 (convert 'map (convert 'vector m0)))
      (error "Map conversion from vector failed (fm0) on iteration ~D" i))
    (unless (equal? fm0 (convert 'map (sort (convert 'vector m0) #'less-than? :key #'car) :input-sorted? t))

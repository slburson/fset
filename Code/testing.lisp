...
(defun Test-2-Relations ()
  ...
  (macrolet ((test (form)
    ...
    (test (equal? (convert 'wb-2-relation (map ('a (set 3 5)) ('b (set 2)) ('c (set 4 7)))
...
(defun fset::test-fset2 ()
  ...
  (macrolet ((test (form)
    ...
    (let ((chm (map ('a 3)))
      ...
      (test (equal? (map-union chm-d (map ('b 4) :default 33))
		    (map ('a 3) ('b 4) :default 33))) ; val-fn discards left default
      ...
      (test (equal? (map-union (map ('b 4) :default 21) chm-d (fn (_x y) (if (= y 42) (values nil ':no-value)
									   y)))
		    (map ('a 3) ('b 4) :no-default)))
      (test (equal? (map-union wbm-d (wb-map ('b 4) :default 13))
		    (wb-map ('b 4) ('c 7) :default 13))) ; val-fn discards left default
      ...
      (test (equal? (map-union (wb-map ('b 4) :default 47) wbm-d (fn (_x y) (if (= y 21) (values nil ':no-value)
									      y)))
		    (wb-map ('b 4) ('c 7) :no-default)))
      ...
      (test (equal? (map-intersection chm chm-d (fn (_x y) (if (= y 42) (values nil ':no-value)
							     y)))
		    chm-no-d))
      (test (equal? (map-intersection wbm wbm-d) wbm))
      ...
      (test (equal? (map-intersection wbm wbm-d (fn (_x y) (if (= y 21) (values nil ':no-value)
							     y)))
		    wbm-no-d))
      (test (equal? (map-intersection chm-d (wb-map ('a 4) :default 17)) (ch-map ('a 4) :default 17)))
      ...
      (test (equal? (compose (ch-map ('a 7) :default 9) (map (7 12) :default 3))

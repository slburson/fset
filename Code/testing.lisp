...
(defun Test-Misc ()
  ...
  (macrolet ((test (form)
	...
	(let* ((lc (list 3))
      ...
      (test (equal? (splice (with-default (seq 'a 'b) 'x) -2 (seq 'c 'd)) (seq 'c 'd 'x 'x 'a 'b)))
      (test (equal? (splice (with-default (seq) 'x) -2 (seq 'c 'd)) (seq 'c 'd 'x 'x)))
      (test (equal? (splice (seq 'a 'b) -1 (seq 'c 'd)) (seq 'c 'd nil 'a 'b)))
      (test (equal? (splice (seq 'a 'b) -1 '(c d)) (seq 'c 'd nil 'a 'b)))
      (test (equal? (splice (with-default (seq) 'x) -1 (seq 'c 'd)) (seq 'c 'd 'x)))
      (test (equal? (splice (with-default (seq) 'x) 0 (seq 'c 'd)) (seq 'c 'd)))
      (test (equal? (default (splice (with-default (seq) 'x) 0 (seq 'c 'd))) 'x))
      (test (equal? (splice (with-default (seq) 'x) 1 (seq 'c 'd)) (seq 'x 'c 'd)))
      (test (equal? (splice (with-default (seq) 'x) 2 (seq 'c 'd)) (seq 'x 'x 'c 'd)))

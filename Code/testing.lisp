...
(defun Test-Misc-4 ()
  (macrolet ((test (form)
    ...
    (test (equal (convert 'list (convert 'map '((a . 3) (b . 4))))
		 '((a . 3) (b . 4))))
    (test (equal (convert 'list (convert 'map '((a . 3) (b . 4)) :key-compare-fn-name 'erapmoc))
		 '((b . 4) (a . 3))))
    (test (equal (convert 'list (convert 'map (seq '(a . 3) '(b . 4))))
		 '((a . 3) (b . 4))))
    (test (equal (convert 'list (convert 'map (seq '(a . 3) '(b . 4)) :key-compare-fn-name 'erapmoc))
		 '((b . 4) (a . 3))))
    (test (equal (convert 'list (convert 'map (seq '(a . 3) '(b . 4)) :input-sorted? t))
		 '((a . 3) (b . 4))))
    (test (equal (convert 'list (convert 'map (seq '(a . 3) '(b . 4)) :input-sorted? t :key-compare-fn-name 'erapmoc))
		 '((b . 4) (a . 3))))
...
(defun Test-Set-Operations (i)
  ...
  (let ((fs0 (empty-wb-set (and (logbitp 0 i) 'erapmoc)))
	(s0 nil)
	(fs1 (empty-wb-set (and (logbitp 1 i) 'erapmoc)))
	(s1 nil))
    (dotimes (j 100)
    ...
    (unless (equal? fs0 (convert 'set (sort s0 #'less-than?) :input-sorted? t))
      (error "Set conversion from sorted list failed (fs0) on iteration ~D" i))
    (unless (equal? fs0 (convert 'set (convert 'seq s0)))
      (error "Set conversion from seq failed (fs0) on iteration ~D" i))
    (unless (equal? fs0 (convert 'set (sort (convert 'seq s0) #'less-than?) :input-sorted? t))
      (error "Set conversion from sorted seq failed (fs0) on iteration ~D" i))
    (unless (equal? fs0 (convert 'set (convert 'vector s0)))
      (error "Set conversion from vector failed (fs0) on iteration ~D" i))
    (unless (equal? fs0 (convert 'set (sort (convert 'vector s0) #'less-than?) :input-sorted? t))
...
(defun Test-Bag-Operations (i)
  (declare (optimize (speed 0) (safety 3) (debug 3)))
  (let ((fb0 (empty-wb-bag (if (logbitp 0 i) 'erapmoc 'compare)))
	(b0 nil)
	(fb1 (empty-wb-bag (if (logbitp 1 i) 'erapmoc 'compare)))
	(b1 nil))
    (dotimes (j 100)
      (let* ((r (Make-My-Integer (random 200)))
	     (tmp (with fb0 r)))
    ...
    (unless (equal? fb0 (convert 'bag (convert 'list fb0) :input-sorted? t))
      (error "Bag conversion from sorted list failed (fb0) on iteration ~D" i))
    (unless (equal? fb0 (convert 'bag (convert 'alist fb0) :input-sorted? t :from-type 'alist))
      (error "Bag conversion from sorted alist failed (fb0) on iteration ~D" i))

    (unless (equal? fb0 (convert 'bag (convert 'seq fb0)))
      (error "Bag conversion from seq failed (fb0) on iteration ~D" i))
    (unless (equal? fb0 (convert 'bag (convert 'seq b0) :pairs? t))
      (error "Bag conversion from seq-of-pairs failed (fb0) on iteration ~D" i))

    (unless (equal? fb0 (convert 'bag (convert 'seq fb0) :input-sorted? t))
      (error "Bag conversion from sorted seq failed (fb0) on iteration ~D" i))
    (unless (equal? fb0 (convert 'bag (sort (convert 'seq b0) #'less-than? :key #'car)
				 :pairs? t :input-sorted? t))
      (error "Bag conversion from sorted seq-of-pairs failed (fb0) on iteration ~D" i))

    (unless (equal? fb0 (convert 'bag (convert 'vector fb0)))
      (error "Bag conversion from vector failed (fb0) on iteration ~D" i))
    (unless (equal? fb0 (convert 'bag (convert 'vector b0) :pairs? t))
      (error "Bag conversion from vector-of-pairs failed (fb0) on iteration ~D" i))
    (unless (equal? fb0 (convert 'bag (convert 'vector fb0) :input-sorted? t))
      (error "Bag conversion from sorted vector failed (fb0) on iteration ~D" i))
    (unless (equal? fb0 (convert 'bag (sort (convert 'vector b0) #'less-than? :key #'car)
				 :pairs? t :input-sorted? t))

			  ...
			  (let ((n (ch-map-node-update transient-id node subnode-idx new-subnode)))
			    ...
			    (setf (ch-map-node-size n)
				  (the fixnum (+ (ch-map-node-size n) (- (ch-bag-tree-size new-subnode)
									 old-subnode-size))))
			    (hash-mixf (ch-bag-node-value-hash n) (hash-unmix (ch-bag-tree-hash-value new-subnode)
									      old-subnode-hash))
			    (setf (ch-map-node-bag-count n) (gen + (ch-map-node-bag-count n) count))
			    (values n t))))
		    ;; Neither entry nor subnode found: make new entry
	   ...
	   (cond
		...
		(cond ((< entries1 entries2) ':less)
		      ...
		      (t
		       ...
		       (let ((default ':equal))
			 (dotimes (i (logcount entries1))
			   (let ((v1 (svref tree1 (+ ch-map-node-header-size (* 2 i))))
				 (m1 (svref tree1 (+ ch-map-node-header-size (1+ (* 2 i)))))
				 (v2 (svref tree2 (+ ch-map-node-header-size (* 2 i))))
				 (m2 (svref tree2 (+ ch-map-node-header-size (1+ (* 2 i)))))
				 ((cmp (funcall compare-fn v1 v2))))

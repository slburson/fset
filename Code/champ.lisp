...
(defun ch-set-tree-with (tree value hash-fn compare-fn &optional (depth 0) replace?)
	  ...
	  (if (null node)
		  ...
		  (if (= hash-shifted wb-hash-shifted)
		    ...
		    (let ((hash-bits (logand hash-shifted champ-hash-level-mask))
			  ...
			  (vector 0 (ash 1 hash-bits) size content-hash
				  (rec node (ash hash-shifted (- champ-hash-bits-per-level)) (1+ depth)))
		...
		(if (logbitp hash-bits entry-mask)
		    ...
		    (let ((ex-value (svref node entry-idx)))
			  ...
			  (if (and replace? (not (eq value ex-value)))
			      (vector-update node entry-idx value)
			    node)
	    ...
	    (t
	     (let ((entries1 (ch-set-node-entry-mask tree1))
	       ...
	       (flet ((add-entry (idx val)
			...
			(ch-set-tree-with tree value hash-fn compare-fn (1+ depth) replace?)))
	     ...
	     (let ((hash-bits (champ-hash-bits (the fixnum (car tree2)) depth))
	       ...
	       (cond ((logbitp hash-bits entry-mask)
			...
			(and (wb-set-tree-contains? (cdr tree2) value compare-fn)
			     (ch-set-tree-with nil value hash-fn compare-fn depth))))
		     ((logbitp hash-bits subnode-mask)
		      (let ((isubnode (1-bits-below hash-bits subnode-mask))
			...
			(do-wb-set-tree-members (x (cdr tree2))
			  (let ((v1? v1 (ch-set-tree-contains? old-subnode x hash-fn compare-fn (1+ depth))))
			    (when v1?
			      (setq result (ch-set-tree-with result v1 hash-fn compare-fn depth)))))
			result))
		     (t nil))))
	    (t
	     (let ((entries1 (ch-set-node-entry-mask tree1))
...
before `isubnode'."
  ...
  (let ((value (ch-set-node-entry node ientry))
	(n (vector-rem-1-ins-1 node (+ ch-set-node-header-size ientry)
			       (- (length node) isubnode) new-subnode)))
    (logandc2f (ch-set-node-entry-mask n) (ash 1 hash-bits))
    (logiorf (ch-set-node-subnode-mask n) (ash 1 hash-bits))
...
;;; We repurpose map trees for bags, except that the slot holding the value hash is
;;; used for the total count (multiplicity) instead.
...
;;; the values.
;;; -- Except in the bag case, where it's just (key-hash . wb-tree).
(deftype ch-map-tree ()
  '(or null cons simple-vector))
...
(defun ch-map-tree-with (tree key value key-hash-fn key-cmp-fn val-hash-fn val-cmp-fn
			 &optional (depth 0) replace?)
  (declare (optimize (speed 1) (safety 2))
	   (type function key-hash-fn key-cmp-fn)
	   (type (or null function) val-hash-fn val-cmp-fn)  ; both null in bag case
	   (type (integer 0 64) depth))
  (let ((key-hash (hash-to-fixnum key key-hash-fn)))
	...
	(let ((hash-bits (logand hash-shifted champ-hash-level-mask)))
	  (if (null node)
	      (vector (ash 1 hash-bits) 0 1 key-hash (and (null val-hash-fn) value) key value)
	    (if (consp node)
		;; Collision node.
		(let ((wb-hash (the fixnum (if (null val-hash-fn) (car node) (caar node))))
		      ((wb-hash-shifted (ash wb-hash (* (- champ-hash-bits-per-level) depth)))))
		  (if (= hash-shifted wb-hash-shifted)
		      ;; Update the collision node.
		      (if (null val-hash-fn)  ; bag case
			  (let ((prev-count (wb-bag-tree-multiplicity (cdr node) key key-cmp-fn))
				((new-wb-tree (wb-bag-tree-with (if replace? (wb-bag-tree-less (cdr node) key key-cmp-fn
											       prev-count)
								  (cdr node))
								key key-cmp-fn (if replace? (gen + value prev-count)
										 value)))))
			    (cons (car node) new-wb-tree))
		    ...
		    (let ((hash-bits (logand hash-shifted champ-hash-level-mask))
			  (wb-hash-bits (logand wb-hash-shifted champ-hash-level-mask))
			  (key-hash (hash-mix wb-hash key-hash))
			  (value-hash (if (null val-hash-fn)
					  (gen + value (wb-bag-tree-total-count (cdr node)))
					(and (cdar node) (hash-mix (cdar node) (hash-to-fixnum value val-hash-fn)))))
			  (size (1+ (if (null val-hash-fn)
					(wb-bag-tree-size (cdr node))
				      (wb-map-tree-size (cdr node))))))
		      (if (= hash-bits wb-hash-bits)
		...
		(if (logbitp hash-bits entry-mask)
		    ...
		    (let ((ex-key (svref node entry-idx))
			  ...
			  (if (or (null val-hash-fn) (null val-cmp-fn))
			      (let ((n (vector-update node (1+ entry-idx) (gen + ex-val value))))
				(when replace?
				  (setf (svref node entry-idx) key))
				(setf (ch-map-node-bag-count n) (gen + (ch-map-node-bag-count n) value))
				n)
			    (if (equal?-cmp value ex-val val-cmp-fn)
				;; Key found, value equal: nothing to do (unless `replace?' requested)
				(if (and replace? (not (and (eq key ex-key) (eq value ex-val))))
				    (vector-update-2 node entry-idx key value)
				  node)
			      ;; Key found, value differs: just update value
			      (let ((n (vector-update-2 node entry-idx key value)))
				(when (ch-map-node-value-hash n)
				  (hash-mixf (ch-map-node-value-hash n)
					     (hash-unmix (hash-to-fixnum value val-hash-fn)
			...
			(let ((hash-shifted (ash hash-shifted (- champ-hash-bits-per-level)))
			      ...
			      ((ex-key-hash-shifted (ash ex-key-hash (* (- champ-hash-bits-per-level) (1+ depth))))
			       ((n2 (if (= hash-shifted ex-key-hash-shifted)
					...
					(if (null val-hash-fn)
					    (cons key-hash
						  (wb-bag-tree-with (wb-bag-tree-with nil ex-key key-cmp-fn ex-val)
								    key key-cmp-fn value))
				      ...
				      (rec (vector (ash 1 (logand ex-key-hash-shifted champ-hash-level-mask))
						   0 1 ex-key-hash (and (null val-hash-fn) ex-val) ex-key ex-val)
					   hash-shifted (1+ depth))))
				;; The `1+' is because we're inserting _after_ `subnode-idx', because the subnodes
				;; are in reverse order.
				...
				(progn
				  (setf (ch-map-node-size n)
					(the fixnum (+ (ch-map-node-size n) (- (ch-bag-tree-size new-subnode)
									       (ch-bag-tree-size subnode)))))
				  (hash-mixf (ch-map-node-key-hash n) (hash-unmix (ch-bag-tree-key-hash new-subnode)
										  (ch-bag-tree-key-hash subnode)))
				  (setf (ch-map-node-bag-count n) (gen + (ch-map-node-bag-count n) value)))
			      (progn
				(setf (ch-map-node-size n)
				      (the fixnum (+ (ch-map-node-size n) (- (ch-map-tree-size new-subnode)
									     (ch-map-tree-size subnode)))))
...
(defun vector-update-2 (vec idx val-0 val-1)
  (declare (optimize (speed 3) (safety 0))
	   (simple-vector vec)
	   (fixnum idx))
  (let ((v (vector-update vec idx val-0)))
    (setf (svref v (1+ idx)) val-1)
    v))

(declaim (ftype (function (ch-map-tree function) fixnum) ch-map-tree-hash-value))
...
(defun ch-bag-tree-hash-value (tree)
  (hash-mix (ch-map-tree-key-hash tree) (ch-bag-tree-total-count tree)))
...
(declaim (ftype (function (ch-map-tree) fixnum) ch-bag-tree-key-hash))
(defun ch-bag-tree-key-hash (tree)
  (declare (optimize (speed 3) (safety 0)))
  (cond ((null tree) 0)
	((consp tree) (car tree))
	(t (ch-map-node-key-hash tree))))
...
(defun ch-map-tree-less (tree key key-hash-fn key-cmp-fn val-hash-fn &optional (depth 0))
  (declare (optimize (speed 1) (safety 2) (debug 3))
	   (type function key-hash-fn key-cmp-fn)
	   (type (or integer function) val-hash-fn)    ; integer in bag case (# to remove)
	   (type (integer 0 64) depth))
  (let ((key-hash (hash-to-fixnum key key-hash-fn))
	(count val-hash-fn))
    (declare (fixnum key-hash))
	...
	(and node
	     (if (consp node)
		 (if (/= (the fixnum (if (integerp count) (car node) (caar node))) key-hash)
		     (values node nil)
		   (if (integerp count)
		       (let ((new-wb-tree (wb-bag-tree-less (cdr node) key key-cmp-fn count)))
			 (if (eq new-wb-tree (cdr node)) node  ; no second value in bag mode
			   (if (= 1 (wb-bag-tree-size new-wb-tree))
			       (let ((x n (wb-bag-tree-arb-pair new-wb-tree)))
		 ...
		 (if (logbitp hash-bits entry-mask)
		     (let ((ex-key (svref node entry-idx)))
		       (cond ((not (equal?-cmp ex-key key key-cmp-fn))
			      (values node nil))
			     ((and (= (logcount entry-mask) 1) (= 0 subnode-mask)
				   (or (not (integerp count)) (gen >= count (svref node (1+ entry-idx)))))
			      (assert (= depth 0))
			      (values nil (svref node (1+ entry-idx))))
			     ((integerp count)
			      (let ((ex-val (svref node (1+ entry-idx)))
				    ((rem-val (gen - ex-val count))))
				(if (gen plusp rem-val)
				    (let ((n (vector-update node (1+ entry-idx) rem-val)))
				      (setf (ch-map-node-bag-count n)
					    (gen - (ch-map-node-bag-count n) count))
				      n)
				  (let ((n (vector-remove-2 node entry-idx)))
				    ...
				    (setf (ch-map-node-bag-count n)
					  (gen - (ch-map-node-bag-count n) ex-val))
				    n))))
			     (t
			      (let ((n (vector-remove-2 node entry-idx)))
				(logandc2f (ch-map-node-entry-mask n) (ash 1 hash-bits))
			   ...
			   (if (eq new-subnode subnode)
			     ...
			     (let ((n (cond ((consp new-subnode)
					    ...
					    ((and (= 0 (ch-map-node-entry-mask new-subnode))
						  ...
						  (consp (ch-map-node-subnode new-subnode 0)))
					     ;; New subnode contains only a collision node: pull it up into this node
					     (vector-update node subnode-idx
				   ...
				   (progn
				     (incf (ch-map-node-size n)
					   (- (ch-bag-tree-size new-subnode) (ch-bag-tree-size subnode)))
				     (hash-mixf (ch-map-node-key-hash n) (hash-unmix (ch-bag-tree-key-hash new-subnode)
										     (ch-bag-tree-key-hash subnode)))
				     (setf (ch-map-node-bag-count n)
					   (gen + (ch-map-node-bag-count n)
						(gen - (ch-bag-tree-total-count new-subnode)
						     (ch-bag-tree-total-count subnode)))))
				 (progn
				   (decf (ch-map-node-size n))
				   (hash-mixf (ch-map-node-key-hash n) (hash-unmix (ch-map-tree-key-hash new-subnode)
										   (ch-map-tree-key-hash subnode)))
...
(defun ch-map-tree-lookup (tree key key-hash-fn key-cmp-fn &optional (depth 0))
  ...
  (let ((key-hash (hash-to-fixnum key key-hash-fn)))
	...
	(and node
	     (if (consp node)
		 (if (not (consp (car node)))
		     (let ((count key-found (wb-bag-tree-multiplicity (cdr node) key key-cmp-fn)))
		       (and (not (eql count 0))
			    (values t count key-found)))
...
(defun ch-map-tree-index (tree key key-hash-fn key-cmp-fn)
  ...
  (let ((key-hash (hash-to-fixnum key key-hash-fn)))
	...
	(and node
	     (if (consp node)
		 (let ((found? idx (if (not (consp (car node)))
				       (wb-bag-tree-rank (cdr node) key key-cmp-fn)
				     (wb-map-tree-rank (cdr node) key key-cmp-fn))))
		   (and found? (the fixnum (+ (the fixnum idx) base))))
	       (let ((hash-bits (logand hash-shifted champ-hash-level-mask))
		     (entry-mask (ch-map-node-entry-mask node)))
		 (if (logbitp hash-bits entry-mask)
    ...
    (cond ((null new-subnode)
	  ...
	  ((and (not (consp new-subnode))
	   ...
	   (let ((ientry (1-bits-below hash-bits (ch-map-node-entry-mask node)))
		 ...
		 ((n (vector-ins-2-rem-1 node (+ ch-map-node-header-size (* 2 ientry)) key val
...
before `isubnode'."
  ...
  (let ((ientry (1-bits-below hash-bits (ch-map-node-entry-mask node)))
	...
	(let ((key val (ch-map-node-entry new-subnode 0))
	      ((n (vector-update-2 node (+ ch-map-node-header-size (* 2 ientry)) key val))))
	  (hash-mixf (ch-map-node-key-hash n)
		     (hash-unmix (hash-to-fixnum key key-hash-fn)
	  ...
	  n)
      (let ((n (vector-rem-2-ins-1 node (+ ch-map-node-header-size (* 2 ientry))
				   (- (length node) isubnode) new-subnode)))
	(logandc2f (ch-map-node-entry-mask n) (ash 1 hash-bits))
	(logiorf (ch-map-node-subnode-mask n) (ash 1 hash-bits))
	      ...
	      (and (test (= 0 (logand entry-mask subnode-mask)))
		   ...
		   (let ((size (logcount entry-mask))
		     ...
		     (and
		       ...
		       (gmap :and (fn (subnode-idx hash-bits)
				    (let ((subnode (svref node (- (length node) 1 subnode-idx))))
					  ...
					  (let ((key-hash (hash-to-fixnum (wb-map-tree-arb-pair (cdr subnode))
									  key-hash-fn))
						(subnode-size (if (null val-hash-fn)
								  (wb-bag-tree-size (cdr subnode))
								(wb-map-tree-size (cdr subnode)))))
					    (test (> subnode-size 1))
					    (incf size subnode-size)
					    (hash-mixf node-key-hash key-hash)
					    (if (null val-hash-fn)
						(incf total-bag-count (wb-bag-tree-total-count (cdr subnode)))
					      (when (cdar subnode)
						(let ((vhash 0))
					    ...
					    (test (= (if (null val-hash-fn) (car subnode) (caar subnode))
						     key-hash)))
					(and (rec subnode (1+ depth) (new-partial-hash hash-bits depth partial-hash))
					     (progn
					       ...
					       (if (null val-hash-fn)
						   (incf total-bag-count (ch-map-node-bag-count subnode))
						 (when (ch-map-node-value-hash node)
		       ...
		       (if (null val-hash-fn)
			   (test (= (ch-map-node-bag-count node) total-bag-count))

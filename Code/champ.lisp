...
(declaim (ftype (function (ch-map-tree) fixnum) ch-bag-tree-hash-value))
...
(defun ch-bag-tree-multiplicity (tree key hash-fn compare-fn &optional (depth 0))
  (let ((found? mult found-key
	  (ch-map/bag-tree-lookup tree key hash-fn compare-fn depth
				  (fn (tree key compare-fn)
				    (let ((mult found-val (wb-bag-tree-multiplicity tree key compare-fn)))
				      (values t mult found-val))))))
    (if found? (values mult found-key)
      (values 0 nil))))
...
(defun ch-bag-node-add-subnode! (n idx subnode key-hash-fn)
  (declare (optimize (speed 3) (safety 0))
	   (type champ-bit-index idx)
	   (type function key-hash-fn))
  (when subnode
	...
	(ch-bag-node-add-entry! n idx (svref subnode ch-bag-node-header-size)
			       (svref subnode (1+ ch-bag-node-header-size)) key-hash-fn)
      (progn
	(when (and (not (consp subnode))
...
(defun ch-bag-node-from-wb (wb-bag-tree key-hash depth)
  (declare (optimize (speed 3) (safety 0)))

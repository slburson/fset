				 ...
				 (let ((wb-hash-shifted (ash (hash-value-fixnum (wb-set-tree-arb (cdr subnode)))
				   ...
				   (if (= hash-shifted wb-hash-shifted)
					 ...
					 (let ((hash-bits (logand hash-shifted champ-hash-level-mask))
					       ...
					       (content-hash (logxor (the fixnum (car subnode)) value-hash)))
...
(defun ch-set-tree-index-element (tree index)
  ...
  (rlabels (rec tree index)
	...
	(let ((len (length tree))
	      ...
	      (entry-idx 0)
	      (subnode-idx 0))
	  (do ((idx 0 (least-1-bit merged-mask)))
	      ((= 0 merged-mask)
	       (error "'Bug in ch-set-tree-index-element'"))
	    (cond ((logbitp idx entry-mask)
		  ...
		  ((logbitp idx subnode-mask)
		   (let ((subnode (svref tree (- len subnode-idx 1)))
		     ...
		     (incf subnode-idx))))
	    (logandc2f merged-mask (ash 1 idx))))))
    (least-1-bit (n)

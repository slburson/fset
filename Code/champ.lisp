...
(defmacro do-bit-indices ((idx-var val &optional result) &body body)
  (let ((val-var (gensymx #:val-)))
    `(let ((,val-var ,val))
       ...
       (do ()
	   ((= 0 ,val-var) ,result)
	 (let ((,idx-var (least-1-bit ,val-var)))
...
(defun ch-set-tree-index-element (tree index)
  ...
  (rlabels (rec tree index)
	...
	(let ((entry-mask (ch-set-node-entry-mask tree))
...
(defun ch-set-tree-iterator-canonicalize (iter)
  ...
  (let ((sp (svref iter 0))
    ...
    ;; Because of TCO, we never have to pop an interior node, only a leaf.
    (when (if (null idx)
	      (wb-set-tree-iterator-done? (svref iter sp))
	    (and (zerop (ch-set-node-subnode-mask node))
		 (= idx (logcount (ch-set-node-entry-mask node)))))
      (decf sp 2)
      (unless (< sp 0)
	...
	(let ((n-entries (logcount (ch-set-node-entry-mask node))))
	  (when (< idx n-entries)
	    (return))
	  (let ((n-subnodes (logcount (ch-set-node-subnode-mask node))))
	    (setq node (svref node (- (length (the simple-vector node)) (- idx n-entries) 1)))
	    (incf idx)
...
(defun ch-map/bag-tree-lookup (tree key key-hash-fn key-cmp-fn depth collision-fn)
  (declare (optimize (speed 3) (safety 0))
	   (type function key-hash-fn key-cmp-fn collision-fn)
	   (type (integer 0 64) depth))
  (let ((key-hash (hash-to-fixnum key key-hash-fn)))
    (declare (fixnum key-hash))
	...
	(and node
	     (if (consp node)
		 (funcall collision-fn (cdr node) key key-cmp-fn)
	       (let ((hash-bits (logand hash-shifted champ-hash-level-mask))
		     (entry-mask (ch-map-node-entry-mask node)))
		 (if (logbitp hash-bits entry-mask)
...
(defun ch-map-tree-index (tree key key-hash-fn key-cmp-fn)
  (ch-map/bag-tree-index tree key key-hash-fn key-cmp-fn #'ch-map-tree-size #'wb-map-tree-rank))

(defun ch-map/bag-tree-index (tree key key-hash-fn key-cmp-fn size-fn collision-fn)
  (declare (optimize (speed 3) (safety 0))
	   (type function key-hash-fn key-cmp-fn size-fn collision-fn))
  (let ((key-hash (hash-to-fixnum key key-hash-fn)))
    (declare (fixnum key-hash))
	...
	(and node
	     (if (consp node)
		 (let ((found? idx (funcall collision-fn (cdr node) key key-cmp-fn)))
		   (and found? (the fixnum (+ (the fixnum idx) base))))
	       (let ((hash-bits (logand hash-shifted champ-hash-level-mask))
		     (entry-mask (ch-map-node-entry-mask node)))
		 (if (logbitp hash-bits entry-mask)
		     ...
		     (and (logbitp hash-bits subnode-mask)
			  (let ((isubnode (1-bits-below hash-bits subnode-mask))
				...
				((subnode-idx (- len 1 isubnode))
				 ((subnode (svref node subnode-idx)))))
			    (dotimes (i isubnode)
			      (incf base (the fixnum (funcall size-fn (svref node (- len 1 i))))))
			    (rec subnode key base (ash hash-shifted (- champ-hash-bits-per-level))))))))))))))

(defun ch-map-tree-index-pair (tree index)
  (ch-map/bag-tree-index-pair tree index #'ch-map-tree-size #'wb-map-tree-rank-pair))

(defun ch-map/bag-tree-index-pair (tree index size-fn collision-fn)
  (declare (optimize (speed 3) (safety 0))
	   (type function size-fn collision-fn))
  (rlabels (rec tree index)
    (rec (tree index)
      (declare (fixnum index))
      (if (consp tree)
	  (funcall collision-fn (cdr tree) index)
	(let ((entry-mask (ch-map-node-entry-mask tree))
	      ((n-entries (logcount entry-mask))))
	  (if (< index n-entries)
	      (let ((idx (+ ch-map-node-header-size (* 2 index))))
		(values (svref tree idx) (svref tree (1+ idx))))
	    (let ((len (length (the simple-vector tree)))
		  (subnode-mask (ch-map-node-subnode-mask tree)))
	      (decf index n-entries)
	      (dotimes (i (logcount subnode-mask))
		(let ((subnode (svref tree (- len i 1)))
		      ((subnode-size (the fixnum (funcall size-fn subnode)))))
		  (when (< index subnode-size)
		    (return (rec subnode index)))
		  (decf index subnode-size))))))))))
...
(defun ch-map-tree-iterator-canonicalize (iter)
  ...
  (let ((sp (svref iter 0))
    ...
    (when (if (null idx)
	    ...
	    (and (zerop (ch-map-node-subnode-mask node))
		 (= idx (logcount (ch-map-node-entry-mask node)))))
      (decf sp 2)
      (unless (< sp 0)
	(setq node (svref iter sp))
	(setq idx (svref iter (1+ sp)))))
	...
	(let ((n-entries (logcount (ch-map-node-entry-mask node))))
	  (when (< idx n-entries)
	    (return))
	  (let ((n-subnodes (logcount (ch-map-node-subnode-mask node))))
	    (setq node (svref node (- (length (the simple-vector node)) (- idx n-entries) 1)))
	    (incf idx)
	    (unless (= idx (+ n-entries n-subnodes)) ; TCO
	      (setf (svref iter (1+ sp)) idx)
...
(defun ch-map-tree-diff (tree1 tree2 key-hash-fn key-cmp-fn val-hash-fn val-cmp-fn)
  (declare (optimize (speed 3) (safety 0))
	   (type function key-hash-fn key-cmp-fn val-hash-fn val-cmp-fn))
  (rlabels (rec tree1 tree2 0)
    (rec (tree1 tree2 depth)
      ...
      (cond ((null tree1) nil)
	    ...
	    ((consp tree1)
	     (if (consp tree2)
		 (if (= (the fixnum (caar tree1)) (the fixnum (caar tree2)))
		     (let ((result (wb-map-tree-diff (cdr tree1) (cdr tree2) key-cmp-fn val-cmp-fn)))
		       (ch-map-node-from-wb result (caar tree1) depth))
		   tree1)
	       (let ((result nil))
		 (do-wb-map-tree-pairs (k v1 (cdr tree1))
		   (let ((v2? v2 (ch-map-tree-lookup tree2 k key-hash-fn key-cmp-fn depth)))
		     (unless (and v2? (equal?-cmp v1 v2 val-cmp-fn))
		       (setq result (wb-map-tree-with result k v1 key-cmp-fn val-cmp-fn)))))
		 (ch-map-node-from-wb result (caar tree1) depth))))
	    ((consp tree2)
	     (let ((result nil))
	       (do-ch-map-tree-pairs (k v1 tree1)
		 (let ((v2? v2 (wb-map-tree-lookup (cdr tree2) k key-cmp-fn)))
		   (unless (and v2? (equal?-cmp v2 v1 val-cmp-fn))
		     (setq result (ch-map-tree-with result k v1 key-hash-fn key-cmp-fn val-hash-fn val-cmp-fn
						    nil depth)))))
	       result))
	    (t
	     (let ((entries1 (ch-map-node-entry-mask tree1))
		   ...
		   (subnodes1 (ch-map-node-subnode-mask tree1))
		   (subnodes2 (ch-map-node-subnode-mask tree2))
		   ((res-len (+ ch-map-node-header-size (* 2 (logcount (logior entries1 subnodes1)))))
		    ((res-n (make-array res-len :initial-element 0))))
		   (ientry1 0)
		   (isubnode1 0)
		   (ientry2 0)
		   ...
		   (cond ((logbitp idx entries1)
			  (let ((key1 val1 (ch-map-node-entry tree1 (postincf ientry1))))
			    (cond ((logbitp idx entries2)
				  ...
				  ((logbitp idx subnodes2)
				   (let ((subnode2 (ch-map-node-subnode tree2 (postincf isubnode2)))
					 ((val2? val2 (subtree-lookup subnode2 key1))))
				     (unless (and val2? (equal?-cmp val1 val2 val-cmp-fn))
				       (add-entry idx key1 val1))))
			 ...
			 ((logbitp idx subnodes1)
			  (let ((subnode1 (ch-map-node-subnode tree1 (postincf isubnode1))))
			    (cond ((logbitp idx subnodes2)
				   (let ((subnode2 (ch-map-node-subnode tree2 (postincf isubnode2)))
					 ((new-sn (rec subnode1 subnode2 (1+ depth)))))
				     (add-subnode idx new-sn)))
				  ((logbitp idx entries2)
				   (let ((key2 val2 (ch-map-node-entry tree2 (postincf ientry2)))
					 ((val1? val1 (subtree-lookup subnode1 key2))))
				     (if (and val1? (equal?-cmp val1 val2 val-cmp-fn))
					 (add-subnode idx (subtree-less subnode1 key2))
				       (add-subnode idx subnode1))))
				  (t (add-subnode idx subnode1)))))
			 ((logbitp idx entries2)
			  (incf ientry2))
			 ((logbitp idx subnodes2)
...
(defun ch-map-tree-restrict-not (map-tree set-tree key-hash-fn key-cmp-fn val-hash-fn)
  ...
  (rlabels (rec map-tree set-tree 0)
    (rec (map-tree set-tree depth)
      (declare (type (integer 0 64) depth))
      (cond ((null map-tree) nil)
	    ((null set-tree) map-tree)
	    ((and (consp map-tree) (consp set-tree))
...
(defun ch-map-tree-compose (tree fn)
  ...
  (cond ((null tree) nil)
	((consp tree)
	 (cons (cons (caar tree) nil) (wb-map-tree-compose (cdr tree) fn)))
	(t
	 (let ((len (length (the simple-vector tree)))
	       (n-entries (logcount (ch-map-node-entry-mask tree)))
	       (n-subnodes (logcount (ch-map-node-subnode-mask tree)))
	       ((new-len (+ ch-map-node-header-size (* 2 n-entries) n-subnodes))
		((n (make-array new-len :initial-element 0)))))
	   (dotimes (i n-entries)
	   ...
	   (dotimes (i n-subnodes)
	     (setf (svref n (- new-len i 1))
		   (ch-map-tree-compose (svref tree (- len i 1)) fn)))
...
(defmacro ch-bag-node-entry-mask (n)
  `(ch-map-node-entry-mask ,n))
(defmacro ch-bag-node-subnode-mask (n)
  `(ch-map-node-subnode-mask ,n))
(defmacro ch-bag-node-size (n)
  `(ch-map-node-size ,n))
(defmacro ch-bag-node-hash-value (n)
  `(ch-map-node-key-hash ,n))
(defmacro ch-bag-node-total-count (n)
  `(ch-map-node-value-hash ,n))
(defconstant ch-bag-node-header-size ch-map-node-header-size)

(defmacro make-ch-bag-node (transient-id entry-mask subnode-mask size key-hash total-count &rest contents)
  `(make-ch-map-node ,transient-id ,entry-mask ,subnode-mask ,size ,key-hash
		     ,total-count	; replaces `value-hash'
		     . ,contents))

(defmacro ch-bag-node-entry (node ientry)
  `(ch-map-node-entry ,node ,ientry))
(defmacro ch-bag-node-subnode (node isubnode)
  `(ch-map-node-subnode ,node ,isubnode))

(declaim (ftype (function (ch-bag-tree) fixnum) ch-bag-tree-hash-value))
(defun ch-bag-tree-hash-value (tree)
  (declare (optimize (speed 3) (safety 0))
	   (type ch-bag-tree tree))
  (cond ((null tree) 0)
	((consp tree) (car tree))
	(t (ch-bag-node-hash-value tree))))

(declaim (ftype (function (ch-bag-tree) fixnum) ch-bag-tree-size))
...
(defun ch-bag-tree-total-count (tree)
  ...
  (cond ((null tree) 0)
	...
	(t (ch-bag-node-total-count tree))))

(defun ch-bag-tree-arb-pair (tree)
  (declare (optimize (speed 3) (safety 0))
	   (type ch-bag-tree tree))
  (cond ((null tree)
	 (error "'ch-bag-tree-arb' called on empty tree"))
	((consp tree)
	 (wb-bag-tree-arb-pair (cdr tree)))
	((= 0 (ch-bag-node-entry-mask tree))
	 ;; If there are no entries, there must be at least one subnode
	 (ch-bag-tree-arb-pair (svref tree (1- (length tree)))))
	(t
	 (values (svref tree ch-bag-node-header-size) (svref tree (1+ ch-bag-node-header-size))))))

(defmacro ch-bag-node-update (transient-id node idx val)
  `(ch-map-node-update ,transient-id ,node ,idx ,val))
(defmacro ch-bag-node-rem-2-ins-1 (transient-id node rem-idx ins-idx ins-val)
  `(ch-map-node-rem-2-ins-1 ,transient-id ,node ,rem-idx ,ins-idx ,ins-val))
(defmacro ch-bag-node-insert-2 (transient-id node idx val-0 val-1)
  `(ch-map-node-insert-2 ,transient-id ,node ,idx ,val-0 ,val-1))
(defmacro ch-bag-node-remove-2 (transient-id node idx)
  `(ch-map-node-remove-2 ,transient-id ,node ,idx))
(defmacro ch-bag-node-ins-2-rem-1 (transient-id node ins-idx ins-0 ins-1 rem-idx)
  `(ch-map-node-ins-2-rem-1 ,transient-id ,node ,ins-idx ,ins-0 ,ins-1 ,rem-idx))
...
if it changed, if `transient-id' is supplied."
  ...
  (let ((value-hash (hash-to-fixnum value hash-fn)))
	...
	(let ((hash-bits (logand hash-shifted champ-hash-level-mask)))
	  (if (null node)
	      (values (make-ch-bag-node transient-id (ash 1 hash-bits) 0 1 value-hash count value count) t)
	    (if (consp node)
		;; Collision node.
		(let ((wb-hash (the fixnum (car node)))
		  ...
		  (if (= hash-shifted wb-hash-shifted)
		    ...
		    (let ((hash-bits (logand hash-shifted champ-hash-level-mask))
			  ...
			  (total-count (gen + (wb-bag-tree-total-count (cdr node)) count)))
		      (values (if (= hash-bits wb-hash-bits)
				  (make-ch-bag-node transient-id 0 (ash 1 hash-bits) size value-hash total-count
						    (rec node (ash hash-shifted (- champ-hash-bits-per-level))
							 (1+ depth)))
				(make-ch-bag-node transient-id (ash 1 hash-bits) (ash 1 wb-hash-bits)
						  size value-hash total-count value count node))
			      t))))
	      ;; Normal case.
	      (let ((entry-mask (ch-bag-node-entry-mask node))
		    ((entry-idx (+ ch-bag-node-header-size (* 2 (1-bits-below hash-bits entry-mask)))))
		    (subnode-mask (ch-bag-node-subnode-mask node))
		    ((subnode-idx (- (length node) 1 (1-bits-below hash-bits subnode-mask)))))
		(if (logbitp hash-bits entry-mask)
		    ;; Entry found
		    (let ((ex-value (svref node entry-idx))
		      ...
		      (if (equal?-cmp value ex-value compare-fn)
			  (let ((n (ch-bag-node-update transient-id node (1+ entry-idx) (gen + ex-count count))))
			    (when replace?
			      (setf (svref n entry-idx) value))
			    (setf (ch-bag-node-total-count n) (gen + (ch-bag-node-total-count n) count))
			    (values n t))
			;; Entry with different value found: make a subnode
			(let ((hash-shifted (ash hash-shifted (- champ-hash-bits-per-level)))
			      (ex-value-hash (hash-to-fixnum ex-value hash-fn))
			      ((ex-value-hash-shifted (ash ex-value-hash (* (- champ-hash-bits-per-level) (1+ depth))))
			       ((n2 (if (= hash-shifted ex-value-hash-shifted)
					...
					(cons value-hash
				      ...
				      ;; Turn the entry into a subnode and recur.
				      (rec (make-ch-bag-node transient-id (ash 1 (logand ex-value-hash-shifted
											 champ-hash-level-mask))
							     0 1 ex-value-hash ex-count ex-value ex-count)
					   hash-shifted (1+ depth))))
				((n (ch-bag-node-rem-2-ins-1 transient-id node entry-idx subnode-idx n2))))))
			  (logandc2f (ch-bag-node-entry-mask n) (ash 1 hash-bits))
			  (logiorf (ch-bag-node-subnode-mask n) (ash 1 hash-bits))
			  (incf (ch-bag-node-size n))
			  (unless (consp n2)
			    (hash-mixf (ch-bag-node-hash-value n) value-hash))
			  (setf (ch-bag-node-total-count n) (gen + (ch-bag-node-total-count n) count))
			  (values n t))))
		  ;; No entry found: check for subnode
		  (if (logbitp hash-bits subnode-mask)
		      ...
		      (let ((subnode (svref node subnode-idx))
			    ((old-subnode-size (ch-bag-tree-size subnode))
			     (old-subnode-hash (ch-bag-tree-hash-value subnode))
			     (new-subnode changed? (rec subnode (ash hash-shifted (- champ-hash-bits-per-level))
							(1+ depth)))))
			(if (not changed?)
			    (values node nil)
			  ;; New subnode
			  (let ((n (ch-bag-node-update transient-id node subnode-idx new-subnode)))
			    ;; If only a count was updated, the subnode won't have changed size.
			    (setf (ch-bag-node-size n)
				  (the fixnum (+ (ch-bag-node-size n) (- (ch-bag-tree-size new-subnode)
									 old-subnode-size))))
			    (hash-mixf (ch-bag-node-hash-value n) (hash-unmix (ch-bag-tree-hash-value new-subnode)
									      old-subnode-hash))
			    (setf (ch-bag-node-total-count n) (gen + (ch-bag-node-total-count n) count))
			    (values n t))))
		    ;; Neither entry nor subnode found: make new entry
		    (let ((n (ch-bag-node-insert-2 transient-id node entry-idx value count)))
		      (logiorf (ch-bag-node-entry-mask n) (ash 1 hash-bits))
		      (incf (ch-bag-node-size n))
		      (hash-mixf (ch-bag-node-hash-value n) value-hash)
		      (setf (ch-bag-node-total-count n) (gen + (ch-bag-node-total-count n) count))
		      (values n t))))))))))))
		   ...
		   (let ((new-wb-tree (wb-bag-tree-less (cdr node) value compare-fn count)))
			 ...
			 (values node nil)
			   ...
			   (let ((rem-val rem-count (wb-bag-tree-arb-pair new-wb-tree))
				 (hash-bits (logand hash-shifted champ-hash-level-mask)))
			     (values (make-ch-bag-node transient-id (ash 1 hash-bits) 0 1
						       value-hash rem-count rem-val rem-count)
				     t))
			 (values (cons (car node) new-wb-tree) t)))))
	       ;; Normal node.
	       (let ((hash-bits (logand hash-shifted champ-hash-level-mask))
		     (entry-mask (ch-bag-node-entry-mask node))
		     ((entry-raw-idx (1-bits-below hash-bits entry-mask))
		      ((entry-idx (+ ch-bag-node-header-size (* 2 entry-raw-idx)))))
		     (subnode-mask (ch-bag-node-subnode-mask node)))
		 (if (logbitp hash-bits entry-mask)
		     ;; Entry found.
		     (let ((ex-value (svref node entry-idx))
			 ...
			 (if (gen > ex-count count)
			   ...
			   (if (and (= (logcount entry-mask) 1) (= 0 subnode-mask))
			       (progn (assert (= depth 0))
				      (values nil t))
			     (let ((n (ch-bag-node-remove-2 transient-id node entry-idx)))
			       (logandc2f (ch-bag-node-entry-mask n) (ash 1 hash-bits))
			       (decf (ch-bag-node-size n))
			       (hash-unmixf (ch-bag-node-hash-value n) value-hash)
			       (setf (ch-bag-node-total-count n)
				     (gen - (ch-bag-node-total-count n) ex-count))
			       (values n t))))))
		   ;; No entry found: check for subnode.
		   (let ((subnode-raw-idx (1-bits-below hash-bits subnode-mask))
			 ((subnode-idx (- (the fixnum (length node)) 1 subnode-raw-idx))))
			 ...
			 (if (not changed?)
			     (values node nil)
			   (let ((n (cond ((consp new-subnode)
					   (ch-bag-node-update transient-id node subnode-idx new-subnode))
					  ((and (= 1 (logcount (ch-bag-node-entry-mask new-subnode)))
						(= 0 (ch-bag-node-subnode-mask new-subnode)))
					   ;; New subnode contains only a single entry: pull it up into this node.
					   (let ((new-entry-idx
						   (+ ch-bag-node-header-size
						      (* 2 (1-bits-below hash-bits entry-mask))))
						 (v c (ch-bag-node-entry new-subnode 0))
						 ((n (ch-bag-node-ins-2-rem-1 transient-id node new-entry-idx v c
									      subnode-idx))))
					     (logiorf (ch-bag-node-entry-mask n) (ash 1 hash-bits))
					     (logandc2f (ch-bag-node-subnode-mask n) (ash 1 hash-bits))
					     n))
					  ((let ((only-subnode))
					     (and (= 0 (ch-bag-node-entry-mask new-subnode))
						  (= 1 (logcount (ch-bag-node-subnode-mask new-subnode)))
						  (consp (setq only-subnode (ch-bag-node-subnode new-subnode 0)))))
					   ;; New subnode contains only a collision node: pull it up into this node.
					   (ch-bag-node-update transient-id node subnode-idx only-subnode))
					  (t (ch-bag-node-update transient-id node subnode-idx new-subnode)))))
			     (incf (ch-bag-node-size n)
				   (- (ch-bag-tree-size new-subnode) old-subnode-size))
			     (hash-mixf (ch-bag-node-hash-value n)
					(hash-unmix (ch-bag-tree-hash-value new-subnode) old-subnode-hash))
			     (setf (ch-bag-node-total-count n)
				   (gen - (ch-bag-node-total-count n)
					(gen - old-subnode-total-count
					     (ch-bag-tree-total-count new-subnode))))
			     (values n t))))))))))))))

(declaim (ftype (function (ch-bag-tree t function function &optional fixnum) integer) ch-bag-tree-multiplicity))
(defun ch-bag-tree-multiplicity (tree value hash-fn compare-fn &optional (depth 0))
  (let ((found? mult found-value
	  (ch-map/bag-tree-lookup tree value hash-fn compare-fn depth
				  (fn (tree value compare-fn)
				    (let ((mult found-val (wb-bag-tree-multiplicity tree value compare-fn)))
				      (values t mult found-val))))))
    (if found? (values mult found-value)
      (values 0 nil))))

(defun ch-bag-compact-tree (tree)
  (ch-map-compact-tree tree))

(defun ch-bag-tree-index (tree key key-hash-fn key-cmp-fn)
  (ch-map/bag-tree-index tree key key-hash-fn key-cmp-fn #'ch-bag-tree-size #'wb-bag-tree-rank))

(defun ch-bag-tree-index-pair (tree index)
  (ch-map/bag-tree-index-pair tree index #'ch-bag-tree-size #'wb-bag-tree-rank-pair))

(defun ch-bag-tree-compare (tree1 tree2 compare-fn)
  (declare (optimize (speed 3) (safety 0))
	   (type function compare-fn))
  (if (eq tree1 tree2) ':equal
    (let ((totct1 (ch-bag-tree-total-count tree1))
	  (totct2 (ch-bag-tree-total-count tree2))
	  (size1 (ch-bag-tree-size tree1))
	  (size2 (ch-bag-tree-size tree2)))
      (cond
	((< size1 size2) ':less)
	((> size1 size2) ':greater)
	((gen < totct1 totct2) ':less)
	((gen > totct1 totct2) ':greater)
	(t
	 (let ((khash1 (ch-bag-tree-hash-value tree1))
	       (khash2 (ch-bag-tree-hash-value tree2)))
	   (cond
	     ((< khash1 khash2) ':less)
	     ((> khash1 khash2) ':greater)
	     ((consp tree1)
	      (if (consp tree2) (wb-bag-tree-compare (cdr tree1) (cdr tree2) compare-fn)
		':greater))
	     ((consp tree2) ':less)
	     (t
	      (let ((entries1 (ch-bag-node-entry-mask tree1))
		    (entries2 (ch-bag-node-entry-mask tree2))
		    (subnodes1 (ch-bag-node-subnode-mask tree1))
		    (subnodes2 (ch-bag-node-subnode-mask tree2)))
		(cond ((< entries1 entries2) ':less)
		      ((> entries1 entries2) ':greater)
		      ((< subnodes1 subnodes2) ':less)
		      ((> subnodes1 subnodes2) ':greater)
		      (t
		       ...
		       (let ((default ':equal))
			 (dotimes (i (logcount entries1))
			   (let ((v1 (svref tree1 (+ ch-bag-node-header-size (* 2 i))))
				 (m1 (svref tree1 (+ ch-bag-node-header-size (1+ (* 2 i)))))
				 (v2 (svref tree2 (+ ch-bag-node-header-size (* 2 i))))
				 (m2 (svref tree2 (+ ch-bag-node-header-size (1+ (* 2 i)))))
				 ((cmp (funcall compare-fn v1 v2))))
			     (when (member cmp '(:less :greater))
			       (return-from ch-bag-tree-compare cmp))
			     (when (eq cmp ':unequal)
			       (setq default ':unequal))
			     (when (gen < m1 m2)
			       (return-from ch-bag-tree-compare ':less))
			     (when (gen > m1 m2)
			       (return-from ch-bag-tree-compare ':greater))))
			 (let ((len1 (length (the simple-vector tree1)))
			       (len2 (length (the simple-vector tree2))))
			   (dotimes (i (logcount subnodes1))
			     (let ((cmp (ch-bag-tree-compare (svref tree1 (- len1 i 1)) (svref tree2 (- len2 i 1))
							     compare-fn)))
			       (ecase cmp
				 (:equal)
				 (:unequal
		...
		(,recur-fn (tree)
		  (when tree
			...
			(do-wb-bag-tree-pairs (,val-var ,count-var (cdr tree))
		      ...
		      (let ((len (the fixnum (length (the simple-vector tree)))))
			(dotimes (i (logcount (ch-bag-node-entry-mask tree)))
			  (let ((idx (+ (* 2 i) ch-bag-node-header-size)))
			    (,body-fn (svref tree idx) (svref tree (1+ idx)))))
			(dotimes (i (logcount (ch-bag-node-subnode-mask tree)))
			  (,recur-fn (svref tree (- len 1 i)))))))))
	 (declare (inline ,body-fn))
	 (,recur-fn ,tree-form))
       ,value-form)))
...
;;; Almost identical to the map iterator, except that collision nodes have to be
;;; handled by the corresponding wb-bag code.
;;; The non-pair case is handled at a higher level.
(defun make-ch-bag-tree-pair-iterator (tree)
  (let ((iter (make-ch-bag-tree-pair-iterator-internal tree)))
    (lambda (op)
      (ecase op
	(:get (ch-bag-tree-pair-iterator-get iter))
	(:done? (ch-bag-tree-pair-iterator-done? iter))
	(:more? (not (ch-bag-tree-pair-iterator-done? iter)))))))

(defun make-ch-bag-tree-pair-iterator-internal (tree)
  (declare (optimize (speed 3) (safety 0)))
  (let ((iter (make-array (+ 1 (* 2 (ceiling (integer-length most-positive-fixnum) champ-hash-bits-per-level))))))
    (if (null tree)
	(setf (svref iter 0) -1)
	...
	(setf (svref iter 1) tree)
	(setf (svref iter 2) 0)
	(ch-bag-tree-pair-iterator-canonicalize iter)))
    iter))

(defun ch-bag-tree-pair-iterator-canonicalize (iter)
  (declare (optimize (speed 3) (safety 0)))
  (let ((sp (svref iter 0))
	((node (svref iter sp))
	 (idx (svref iter (1+ sp)))))
    ...
    ;; Because of TCO, we never have to pop an interior node, only a leaf.
    (when (if (null idx)
	      (wb-bag-tree-pair-iterator-done? (svref iter sp))
	    (and (zerop (ch-bag-node-subnode-mask node))
		 (= idx (logcount (ch-bag-node-entry-mask node)))))
      (decf sp 2)
      (unless (< sp 0)
	(setq node (svref iter sp))
	(setq idx (svref iter (1+ sp)))))
	...
	(when (null idx)
	  (return))
	(let ((n-entries (logcount (ch-bag-node-entry-mask node))))
	  (when (< idx n-entries)
	    (return))
	  (let ((n-subnodes (logcount (ch-bag-node-subnode-mask node))))
	    (setq node (svref node (- (length (the simple-vector node)) (- idx n-entries) 1)))
	    (incf idx)
	    (unless (= idx (+ n-entries n-subnodes)) ; TCO
	      (setf (svref iter (1+ sp)) idx)
	      (incf sp 2))
	    (if (consp node)
		(setq node (make-wb-bag-tree-pair-iterator-internal (cdr node))
		      idx nil)
	      (setq idx 0))
	    (setf (svref iter sp) node)
	    (setf (svref iter (1+ sp)) idx)))))
...
(defun ch-bag-tree-pair-iterator-done? (iter)
  (declare (optimize (speed 3) (safety 0)))
  (< (the fixnum (svref iter 0)) 0))

(defun ch-bag-tree-pair-iterator-get (iter)
  (declare (optimize (speed 3) (safety 0)))
  (let ((sp (svref iter 0)))
    (declare (fixnum sp))
    (if (= sp 0)
	...
	(declare (type (or null fixnum) idx))
	(if (null idx)
	    (let ((key val (wb-bag-tree-pair-iterator-get node)))
	      (ch-bag-tree-pair-iterator-canonicalize iter)
	      (values key val t))
	  (let ((entry-idx (+ ch-bag-node-header-size (the fixnum (* 2 idx)))))
	    (setf (svref iter (1+ sp)) (1+ idx))
	    (ch-bag-tree-pair-iterator-canonicalize iter)
	    (values (svref node entry-idx) (svref node (1+ entry-idx))
		    t)))))))
...
(defun ch-bag-tree-pair-fun-iter (tree)
  (declare (optimize (speed 3) (safety 0)))
  (rlabels (walk tree (lambda (op)
			(ecase op
			  (:first (values nil nil))
    ...
    (walk (node cont)
      (cond ((null node) cont)
	    ((consp node)
	     (wb-bag-tree-pair-fun-iter (cdr node) cont))
	    (t
	     (let ((n-entries (logcount (ch-bag-node-entry-mask node)))
		   (n-subnodes (logcount (ch-bag-node-subnode-mask node))))
	       (rlabels (entry 0)
		 (entry (i)
		   (declare (fixnum i))
		   (if (< i n-entries)
		       (lambda (op)
			 (ecase op
			   (:first (let ((entry-idx (+ ch-bag-node-header-size (the fixnum (* 2 i)))))
				     (values (svref node entry-idx) (svref node (1+ entry-idx)) t)))
			   (:rest (entry (1+ i)))
			   (:empty? nil)
			   (:more? t)))
		 ...
		 (subnode (i)
		   (declare (fixnum i))
		   (if (< i n-subnodes)
		       (walk (svref node (- (+ ch-bag-node-header-size (* 2 n-entries) n-subnodes)
					    i 1))
			     (lambda (op) (funcall (the function (subnode (1+ i))) op)))
		     cont)))))))))

(defun ch-bag-tree-pair-rev-fun-iter (tree)
  (declare (optimize (speed 3) (safety 0)))
  (rlabels (walk tree (lambda (op)
			(ecase op
			  (:first (values nil nil))
    ...
    (walk (node cont)
      (cond ((null node) cont)
	    ((consp node)
	     (wb-bag-tree-pair-rev-fun-iter (cdr node) cont))
	    (t
	     (let ((n-entries (logcount (ch-bag-node-entry-mask node)))
		   (n-subnodes (logcount (ch-bag-node-subnode-mask node))))
	       (rlabels (subnode (1- n-subnodes))
...
;;; --------------------------------
;;; Union, intersection, sum, product, bag-difference, subbag?, disjoint?

(declaim (inline ch-bag-node-add-entry!))
(defun ch-bag-node-add-entry! (n idx value count hash-fn)
  (ch-map-node-add-entry! n idx value count hash-fn)
  (setf (ch-bag-node-total-count n)
	(gen + (ch-bag-node-total-count n) count)))

(declaim (inline ch-bag-node-add-subnode!))
(defun ch-bag-node-add-subnode! (n idx subnode hash-fn)
  (declare (optimize (speed 3) (safety 0))
	   (type champ-bit-index idx)
	   (type function hash-fn))
  (when subnode
    (if (and (not (consp subnode))
	     (= 1 (logcount (ch-bag-node-entry-mask subnode)))
	     (= 0 (ch-bag-node-subnode-mask subnode)))
	;; `subnode' contains only an entry -- pull it up.
	(ch-bag-node-add-entry! n idx (svref subnode ch-bag-node-header-size)
			       (svref subnode (1+ ch-bag-node-header-size)) hash-fn)
      (progn
	(when (and (not (consp subnode))
		   (= 0 (ch-bag-node-entry-mask subnode))
		   (= 1 (logcount (ch-bag-node-subnode-mask subnode)))
		   (consp (svref subnode (1- (length subnode)))))
	  ;; `subnode' contains only a collision subnode -- pull it up.
	  (setq subnode (svref subnode (1- (length subnode)))))
	(setf (svref n (- (length (the simple-vector n))
			  (logcount (ch-bag-node-subnode-mask n)) 1))
	      subnode)
	(logiorf (ch-bag-node-subnode-mask n) (ash 1 idx))
	(incf (ch-bag-node-size n) (ch-bag-tree-size subnode))
	(hash-mixf (ch-bag-node-hash-value n) (ch-bag-tree-hash-value subnode))
	(setf (ch-bag-node-total-count n)
	      (gen + (ch-bag-node-total-count n) (ch-bag-tree-total-count subnode)))))))

(defmacro ch-bag-compact-node (n)
  `(ch-map-compact-node ,n))

(defun ch-bag-tree-union (tree1 tree2 hash-fn compare-fn)
  (declare (optimize (speed 3) (safety 0))
	   (type function hash-fn compare-fn))
  (rlabels (rec tree1 tree2 0)
    (rec (tree1 tree2 depth)
      ...
      (cond ((null tree1) tree2)
	    ...
	    ((and (consp tree1) (consp tree2))
	     (if (= (the fixnum (car tree1)) (the fixnum (car tree2)))
		 (let ((result (wb-bag-tree-union (cdr tree1) (cdr tree2) compare-fn)))
		   (ch-bag-node-from-wb result (car tree1) depth))
	       (let ((bits1 (champ-hash-bits (the fixnum (car tree1)) depth))
		     (bits2 (champ-hash-bits (the fixnum (car tree2)) depth))
		     (size (+ (wb-bag-tree-size (cdr tree1)) (wb-bag-tree-size (cdr tree2))))
		     (count (gen + (wb-bag-tree-total-count (cdr tree1)) (wb-bag-tree-total-count (cdr tree2))))
		     (hash (hash-mix (car tree1) (car tree2))))
		 (declare (fixnum size))
		 (if (= bits1 bits2)
		     (vector 0 (ash 1 bits1) size hash count (rec tree1 tree2 (1+ depth)))
		   (let ((subnode0 subnode1 (swap-if (> bits1 bits2) tree1 tree2)))
		     (vector 0 (logior (ash 1 bits1) (ash 1 bits2)) size hash count subnode1 subnode0))))))
	    ((or (consp tree1) (consp tree2))
	     (let ((tree1-collision? (consp tree1))
		   ((norm-node coll-node (swap-if tree1-collision? tree1 tree2))
		    ((hash-bits (champ-hash-bits (the fixnum (car coll-node)) depth))
		     (entry-mask (ch-bag-node-entry-mask norm-node))
		     (subnode-mask (ch-bag-node-subnode-mask norm-node)))))
	       (declare (type simple-vector norm-node))
	       (cond ((logbitp hash-bits entry-mask)
		      (let ((ientry (1-bits-below hash-bits entry-mask))
			    ((norm-val norm-count (ch-bag-node-entry norm-node ientry))
			     ((coll-count coll-val (wb-bag-tree-multiplicity (cdr coll-node) norm-val compare-fn))
			      ((new-count (gen max norm-count coll-count))
			       ((new-subnode
				  (if (/= coll-count 0)
				      (ch-bag-tree-with coll-node (if tree1-collision? coll-val norm-val)
							(gen - new-count coll-count) hash-fn compare-fn nil (1+ depth)
							(not tree1-collision?))
				    (ch-bag-tree-with coll-node norm-val norm-count hash-fn compare-fn nil (1+ depth)
						      (not tree1-collision?)))))))))
			(ch-bag-node-change-entry-to-subnode norm-node hash-bits new-subnode hash-fn)))
		     ((logbitp hash-bits subnode-mask)
		      (let ((isubnode (1-bits-below hash-bits subnode-mask))
			    ((old-subnode (ch-bag-node-subnode norm-node isubnode))))
			(ch-bag-node-replace-subnode norm-node hash-bits
						     (if tree1-collision? (rec coll-node old-subnode (1+ depth))
						       (rec old-subnode coll-node (1+ depth))))))
		     (t
		      (ch-bag-node-insert-subnode norm-node hash-bits coll-node)))))
	    (t
	     (let ((entries1 (ch-bag-node-entry-mask tree1))
		   (entries2 (ch-bag-node-entry-mask tree2))
		   (subnodes1 (ch-bag-node-subnode-mask tree1))
		   ...
		   ((all-entries (logior entries1 entries2))
		    (all-subnodes (logior subnodes1 subnodes2))
		    ((everything (logior all-entries all-subnodes))
		     ((res-len (+ ch-bag-node-header-size (* 2 (logcount all-entries)) (logcount all-subnodes)))
		      ((res-n (make-array res-len :initial-element 0))))))
		   (ientry1 0)
		   (isubnode1 0)
		   (ientry2 0)
	       ...
	       (declare (fixnum ientry1 isubnode1 ientry2 isubnode2))
	       (flet ((add-entry (idx key val)
			(ch-bag-node-add-entry! res-n idx key val hash-fn))
		      (add-subnode (idx subnode)
			(ch-bag-node-add-subnode! res-n idx subnode hash-fn)
			nil)  ; hush, SBCL
		      (subtree-lookup (tree key)
			(ch-bag-tree-multiplicity tree key hash-fn compare-fn (1+ depth)))
		      (subtree-with (subtree value count &optional replace?)
			(values (ch-bag-tree-with subtree value count hash-fn compare-fn nil (1+ depth) replace?))))
		 (do-bit-indices (idx everything)
		   (cond ((and (logbitp idx entries1) (logbitp idx entries2))
			  (let ((value1 count1 (ch-bag-node-entry tree1 (postincf ientry1)))
				(value2 count2 (ch-bag-node-entry tree2 (postincf ientry2))))
			    (if (equal?-cmp value1 value2 compare-fn)
				(add-entry idx value1 (gen max count1 count2))
			      (add-subnode idx (subtree-with (subtree-with nil value1 count1) value2 count2)))))
			 ((logbitp idx entries1)
			  (let ((value1 count1 (ch-bag-node-entry tree1 (postincf ientry1))))
			    (if (logbitp idx subnodes2)
				(let ((subnode2 (ch-bag-node-subnode tree2 (postincf isubnode2)))
				      ((count2 (subtree-lookup subnode2 value1))))
				  (add-subnode idx (subtree-with subnode2 value1 (gen max 0 (gen - count1 count2)) t)))
			      (add-entry idx value1 count1))))
			 ((logbitp idx entries2)
			  (let ((value2 count2 (ch-bag-node-entry tree2 (postincf ientry2))))
			    (if (logbitp idx subnodes1)
				(let ((subnode1 (ch-bag-node-subnode tree1 (postincf isubnode1)))
				      ((count1 value1 (subtree-lookup subnode1 value2))))
				  (if (/= count1 0)
				      (add-subnode idx (subtree-with subnode1 value1 (gen max 0 (gen - count2 count1))))
				    (add-subnode idx (subtree-with subnode1 value2 count2))))
			      (add-entry idx value2 count2))))
			 ((and (logbitp idx subnodes1) (logbitp idx subnodes2))
			  (add-subnode idx (rec (ch-bag-node-subnode tree1 (postincf isubnode1))
						(ch-bag-node-subnode tree2 (postincf isubnode2))
						(1+ depth))))
			 ((logbitp idx subnodes1)
			  (add-subnode idx (ch-bag-node-subnode tree1 (postincf isubnode1))))
			 ((logbitp idx subnodes2)
			  (add-subnode idx (ch-bag-node-subnode tree2 (postincf isubnode2))))
			 (t (error "Bug in 'ch-bag-tree-union'")))))
	       ;; When two entries meet and become a subnode, we no longer need the value slot.
	       (ch-bag-compact-node res-n)))))))

(defun ch-bag-tree-sum (tree1 tree2 hash-fn compare-fn)
  (declare (optimize (speed 3) (safety 0))
	   (type function hash-fn compare-fn))
  (rlabels (rec tree1 tree2 0)
    (rec (tree1 tree2 depth)
      ...
      (cond ((null tree1) tree2)
	    ...
	    ((and (consp tree1) (consp tree2))
	     (if (= (the fixnum (car tree1)) (the fixnum (car tree2)))
		 (let ((result (wb-bag-tree-sum (cdr tree1) (cdr tree2) compare-fn)))
		   (ch-bag-node-from-wb result (car tree1) depth))
	       (let ((bits1 (champ-hash-bits (the fixnum (car tree1)) depth))
		     (bits2 (champ-hash-bits (the fixnum (car tree2)) depth))
		     (size (+ (wb-bag-tree-size (cdr tree1)) (wb-bag-tree-size (cdr tree2))))
	       ...
	       (cond ((logbitp hash-bits entry-mask)
		      (let ((ientry (1-bits-below hash-bits entry-mask))
			    ((norm-val norm-count (ch-bag-node-entry norm-node ientry))
				 ...
				 (if (/= coll-count 0)
				     (ch-bag-tree-with coll-node (if tree1-collision? coll-val norm-val)
						       norm-count hash-fn compare-fn nil (1+ depth)
						       (not tree1-collision?))
				   (ch-bag-tree-with coll-node norm-val norm-count hash-fn compare-fn nil (1+ depth)
						     (not tree1-collision?))))))))
			(ch-bag-node-change-entry-to-subnode norm-node hash-bits new-subnode hash-fn)))
		 ...
		 (do-bit-indices (idx everything)
		   (cond ((and (logbitp idx entries1) (logbitp idx entries2))
			  (let ((value1 count1 (ch-bag-node-entry tree1 (postincf ientry1)))
				(value2 count2 (ch-bag-node-entry tree2 (postincf ientry2))))
			    (if (equal?-cmp value1 value2 compare-fn)
				(add-entry idx value1 (gen + count1 count2))
			      (add-subnode idx (subtree-with (subtree-with nil value1 count1) value2 count2)))))
			 ((logbitp idx entries1)
			  (let ((value1 count1 (ch-bag-node-entry tree1 (postincf ientry1))))
			    (if (logbitp idx subnodes2)
				(let ((subnode2 (ch-bag-node-subnode tree2 (postincf isubnode2))))
				  (add-subnode idx (subtree-with subnode2 value1 count1 t)))
			      (add-entry idx value1 count1))))
			 ((logbitp idx entries2)
			  (let ((value2 count2 (ch-bag-node-entry tree2 (postincf ientry2))))
			    (if (logbitp idx subnodes1)
				(let ((subnode1 (ch-bag-node-subnode tree1 (postincf isubnode1)))
				      ((count1 value1 (subtree-lookup subnode1 value2))))
				  (if (/= count1 0)
				      (add-subnode idx (subtree-with subnode1 value1 count2))
				    (add-subnode idx (subtree-with subnode1 value2 count2))))
			      (add-entry idx value2 count2))))
			 ((and (logbitp idx subnodes1) (logbitp idx subnodes2))
			  (add-subnode idx (rec (ch-bag-node-subnode tree1 (postincf isubnode1))
			 ...
			 ((logbitp idx subnodes2)
			  (add-subnode idx (ch-bag-node-subnode tree2 (postincf isubnode2))))
			 (t (error "Bug in 'ch-bag-tree-sum'")))))
	       ;; When two entries meet and become a subnode, we no longer need the value slot.
	       (ch-bag-compact-node res-n)))))))

(defun ch-bag-node-from-wb (wb-bag-tree hash depth)
  (declare (optimize (speed 3) (safety 0)))
  (case (wb-bag-tree-size wb-bag-tree)
    (0 nil)
    (1 (let ((k v (wb-bag-tree-arb-pair wb-bag-tree)))
	 (vector (ash 1 (champ-hash-bits hash depth))
		 0 1 hash nil k v)))
    (t (cons hash wb-bag-tree))))

(defun ch-bag-node-change-entry-to-subnode (node hash-bits new-subnode hash-fn)
  "Returns a new node with entry `ientry' removed, and `new-subnode' inserted just
before `isubnode'."
  (declare (optimize (speed 3) (safety 0))
	   (type simple-vector node)
	   (type champ-bit-index hash-bits)
	   (type function hash-fn))
  (let ((ientry (1-bits-below hash-bits (ch-bag-node-entry-mask node)))
	(isubnode (1-bits-below hash-bits (ch-bag-node-subnode-mask node)))
	((old-val old-count (ch-bag-node-entry node ientry))))
    (let ((n (ch-bag-node-rem-2-ins-1 nil node (+ ch-bag-node-header-size (* 2 ientry))
				      (- (length node) isubnode 1) new-subnode)))
      (logandc2f (ch-bag-node-entry-mask n) (ash 1 hash-bits))
      (logiorf (ch-bag-node-subnode-mask n) (ash 1 hash-bits))
      (incf (ch-bag-node-size n) (1- (ch-bag-tree-size new-subnode)))
      (hash-mixf (ch-bag-node-hash-value n)
		 (hash-unmix (ch-bag-tree-hash-value new-subnode)
			     (hash-to-fixnum old-val hash-fn)))
      (setf (ch-bag-node-total-count n) (gen + (ch-bag-node-total-count n)
					     (gen - (ch-bag-tree-total-count new-subnode) old-count)))
      n)))

(defun ch-bag-node-replace-subnode (node hash-bits new-subnode)
  "Returns a new node with `new-subnode' replacing the subnode at `isubnode'."
  (declare (optimize (speed 3) (safety 0))
	   (type simple-vector node)
	   (type champ-bit-index hash-bits))
  (let ((isubnode (1-bits-below hash-bits (ch-bag-node-subnode-mask node)))
	((old-subnode (ch-bag-node-subnode node isubnode))))
    (let ((n (vector-update node (- (length node) isubnode 1) new-subnode)))
      (declare (type simple-vector n))
      (setf (svref n (- (length n) isubnode 1)) new-subnode)
      (incf (ch-bag-node-size n) (- (ch-bag-tree-size new-subnode) (ch-bag-tree-size old-subnode)))
      (hash-mixf (ch-bag-node-hash-value n)
		 (hash-unmix (ch-bag-tree-hash-value new-subnode)
			     (ch-bag-tree-hash-value old-subnode)))
      (setf (ch-bag-node-total-count n)
	    (gen + (ch-bag-node-total-count n)
		 (gen - (ch-bag-tree-total-count new-subnode) (ch-bag-tree-total-count old-subnode))))
      n)))

(defun ch-bag-node-insert-subnode (node hash-bits new-subnode)
  "Returns a new node with `new-subnode' inserted just before `isubnode'."
  (declare (optimize (speed 3) (safety 0))
	   (type simple-vector node)
	   (type champ-bit-index hash-bits))
  (let ((isubnode (1-bits-below hash-bits (ch-bag-node-subnode-mask node)))
	((n (vector-insert node (- (length node) isubnode) new-subnode))))
    (logiorf (ch-bag-node-subnode-mask n) (ash 1 hash-bits))
    (incf (ch-bag-node-size n) (ch-bag-tree-size new-subnode))
    (hash-mixf (ch-bag-node-hash-value n) (ch-bag-tree-hash-value new-subnode))
    (setf (ch-bag-node-total-count n)
	  (gen + (ch-bag-node-total-count n) (ch-bag-tree-total-count new-subnode)))
    n))

(defun ch-bag-tree-intersection (tree1 tree2 hash-fn compare-fn)
  (declare (optimize (speed 3) (safety 0))
	   (type function hash-fn compare-fn))
  (rlabels (rec tree1 tree2 0)
    (rec (tree1 tree2 depth)
      ...
      (cond ((or (null tree1) (null tree2)) nil)
	    ...
	    ((consp tree1)
	     (if (consp tree2)
		 (and (= (the fixnum (car tree1)) (the fixnum (car tree2)))
		      (let ((result (wb-bag-tree-intersect (cdr tree1) (cdr tree2) compare-fn)))
			(ch-bag-node-from-wb result (car tree1) depth)))
	       ;; &&& Unify this with the tree2-collision case, as I did in `union'.
	       (let ((result nil))
		 (do-wb-bag-tree-pairs (v1 n1 (cdr tree1))
		   (let ((n2 (ch-bag-tree-multiplicity tree2 v1 hash-fn compare-fn depth)))
		     (when (/= n2 0)
		       (setq result (ch-bag-tree-with result v1 (gen min n1 n2) hash-fn compare-fn nil depth)))))
		 result)))
	    ((consp tree2)
	     (let ((hash-bits (champ-hash-bits (the fixnum (car tree2)) depth))
		   (entry-mask (ch-bag-node-entry-mask tree1))
		   (subnode-mask (ch-bag-node-subnode-mask tree1)))
	       (cond ((logbitp hash-bits entry-mask)
		      (let ((ientry (1-bits-below hash-bits entry-mask))
			    ((v1 n1 (ch-bag-node-entry tree1 ientry))
			     ((n2 (wb-bag-tree-multiplicity (cdr tree2) v1 compare-fn)))))
			(and (/= n2 0)
			     (ch-bag-tree-with nil v1 (gen min n1 n2) hash-fn compare-fn nil depth))))
		     ((logbitp hash-bits subnode-mask)
		      (let ((isubnode (1-bits-below hash-bits subnode-mask))
			    ((old-subnode (ch-bag-node-subnode tree1 isubnode)))
			    (result nil))
			(do-wb-bag-tree-pairs (v2 n2 (cdr tree2))
			  (let ((n1 v1 (ch-bag-tree-multiplicity old-subnode v2 hash-fn compare-fn (1+ depth))))
			    (when (/= n1 0)
			      (setq result (ch-bag-tree-with result v1 (gen min n1 n2) hash-fn compare-fn nil depth)))))
			result))
		     (t nil))))
	    (t
	     (let ((entries1 (ch-bag-node-entry-mask tree1))
		   ...
		   ((combined-mask (logand (logior entries1 subnodes1) (logior entries2 subnodes2)))
		    ;; The result can be longer than either input, because the intersection of two subnodes
		    ;; could be an entry.
		    ((res-len (+ ch-bag-node-header-size (* 2 (logcount combined-mask))))
		     ((res-n (make-array res-len :initial-element 0))))))
	       (flet ((add-entry (idx value count)
			(ch-bag-node-add-entry! res-n idx value count hash-fn))
		      (add-subnode (idx subnode)
			(ch-bag-node-add-subnode! res-n idx subnode hash-fn))
		      (subtree-lookup (tree value)
			(ch-bag-tree-multiplicity tree value hash-fn compare-fn (1+ depth))))
		 (do-bit-indices (idx combined-mask)
		   (cond ((and (logbitp idx entries1) (logbitp idx entries2))
			  (let ((value1 count1 (ch-bag-node-entry tree1 (1-bits-below idx entries1)))
				(value2 count2 (ch-bag-node-entry tree2 (1-bits-below idx entries2))))
			    (when (equal?-cmp value1 value2 compare-fn)
			      (add-entry idx value1 (gen min count1 count2)))))
			 ((and (logbitp idx entries1) (logbitp idx subnodes2))
			  (let ((value1 count1 (ch-bag-node-entry tree1 (1-bits-below idx entries1)))
				(subnode2 (ch-bag-node-subnode tree2 (1-bits-below idx subnodes2)))
				((count2 (subtree-lookup subnode2 value1))))
			    (when (/= count2 0)
			      (add-entry idx value1 (gen min count1 count2)))))
			 ((and (logbitp idx subnodes1) (logbitp idx entries2))
			  (let ((value2 count2 (ch-bag-node-entry tree2 (1-bits-below idx entries2)))
				(subnode1 (ch-bag-node-subnode tree1 (1-bits-below idx subnodes1)))
				((count1 value1 (subtree-lookup subnode1 value2))))
			    (when (/= count1 0)
			      (add-entry idx value1 (gen min count1 count2)))))
			 ((and (logbitp idx subnodes1) (logbitp idx subnodes2))
			  (add-subnode idx (rec (ch-bag-node-subnode tree1 (1-bits-below idx subnodes1))
						(ch-bag-node-subnode tree2 (1-bits-below idx subnodes2))
						(1+ depth))))))
		 (ch-bag-compact-node res-n))))))))

(defun ch-bag-tree-product (tree1 tree2 hash-fn compare-fn)
  (declare (optimize (speed 3) (safety 0))
	   (type function hash-fn compare-fn))
  (rlabels (rec tree1 tree2 0)
    (rec (tree1 tree2 depth)
      ...
      (cond ((or (null tree1) (null tree2)) nil)
	    ...
	    ((consp tree1)
	     (if (consp tree2)
		 (and (= (the fixnum (car tree1)) (the fixnum (car tree2)))
		      (let ((result (wb-bag-tree-product (cdr tree1) (cdr tree2) compare-fn)))
			(ch-bag-node-from-wb result (car tree1) depth)))
	       ;; &&& Unify this with the tree2-collision case, as I did in `union'.
	       (let ((result nil))
		 (do-wb-bag-tree-pairs (v1 n1 (cdr tree1))
		   (let ((n2 (ch-bag-tree-multiplicity tree2 v1 hash-fn compare-fn depth)))
		     (when (/= n2 0)
		       (setq result (ch-bag-tree-with result v1 (gen * n1 n2) hash-fn compare-fn nil depth)))))
		 result)))
	    ((consp tree2)
	     (let ((hash-bits (champ-hash-bits (the fixnum (car tree2)) depth))
		   (entry-mask (ch-bag-node-entry-mask tree1))
	       ...
	       (cond ((logbitp hash-bits entry-mask)
		      (let ((ientry (1-bits-below hash-bits entry-mask))
			    ((v1 n1 (ch-bag-node-entry tree1 ientry))
			     ((n2 (wb-bag-tree-multiplicity (cdr tree2) v1 compare-fn)))))
			(and (/= n2 0)
			     (ch-bag-tree-with nil v1 (gen * n1 n2) hash-fn compare-fn nil depth))))
		     ((logbitp hash-bits subnode-mask)
		      (let ((isubnode (1-bits-below hash-bits subnode-mask))
			    ((old-subnode (ch-bag-node-subnode tree1 isubnode)))
			    (result nil))
...
(defun ch-bag-tree-diff (tree1 tree2 hash-fn compare-fn)
    ...
    (rec (tree1 tree2 depth)
      ...
      (cond ((null tree1) nil)
	    ...
	    ((consp tree1)
	     (if (consp tree2)
		 (if (= (the fixnum (car tree1)) (the fixnum (car tree2)))
		     (let ((result (wb-bag-tree-diff (cdr tree1) (cdr tree2) compare-fn)))
		       (ch-bag-node-from-wb result (car tree1) depth))
		   tree1)
	       (let ((result nil))
		 (do-wb-bag-tree-pairs (v1 n1 (cdr tree1))
		   (let ((n2 (ch-bag-tree-multiplicity tree2 v1 hash-fn compare-fn depth)))
		     (unless (gen >= n2 n1)
		       (setq result (wb-bag-tree-with result v1 compare-fn (gen - n1 n2))))))
		 (ch-bag-node-from-wb result (car tree1) depth))))
	    ((consp tree2)
	     (let ((result nil))
	       (do-ch-bag-tree-pairs (v1 n1 tree1)
		 (let ((n2 (wb-bag-tree-multiplicity (cdr tree2) v1 compare-fn)))
		   (unless (gen >= n2 n1)
		     (setq result (ch-bag-tree-with result v1 (gen - n1 n2) hash-fn compare-fn nil depth)))))
	       result))
	    (t
	     (let ((entries1 (ch-bag-node-entry-mask tree1))
		   ...
		   (subnodes1 (ch-bag-node-subnode-mask tree1))
		   (subnodes2 (ch-bag-node-subnode-mask tree2))
		   ((res-len (+ ch-bag-node-header-size (* 2 (logcount (logior entries1 subnodes1)))))
		    ((res-n (make-array res-len :initial-element 0))))
		   (ientry1 0)
		   (isubnode1 0)
		   (ientry2 0)
	       ...
	       (flet ((add-entry (idx value count)
			(ch-bag-node-add-entry! res-n idx value count hash-fn))
		      (add-subnode (idx subnode)
			(ch-bag-node-add-subnode! res-n idx subnode hash-fn))
		      (subtree-less (subtree value count)
			(ch-bag-tree-less subtree value count hash-fn compare-fn nil (1+ depth)))
		      (subtree-lookup (subtree value)
			(ch-bag-tree-multiplicity subtree value hash-fn compare-fn (1+ depth))))
		 (do-bit-indices (idx (logior entries1 subnodes1 entries2 subnodes2))
		   (cond ((logbitp idx entries1)
			  (let ((value1 count1 (ch-bag-node-entry tree1 (postincf ientry1))))
			    (cond ((logbitp idx entries2)
			 ...
			 ((logbitp idx subnodes1)
			  (let ((subnode1 (ch-bag-node-subnode tree1 (postincf isubnode1))))
			    (cond ((logbitp idx subnodes2)
				  ...
				  ((logbitp idx entries2)
				   (let ((value2 count2 (ch-bag-node-entry tree2 (postincf ientry2)))
					 ((count1 (subtree-lookup subnode1 value2))))
				     (if (/= count1 0)
					 (add-subnode idx (subtree-less subnode1 value2 count2))
				       (add-subnode idx subnode1))))
				  (t (add-subnode idx subnode1)))))
			 ((logbitp idx entries2)
			  (incf ientry2))
			 ((logbitp idx subnodes2)
			  (incf isubnode2))
			 (t (error "Bug in ch-bag-tree-diff")))))
	       (ch-bag-compact-node res-n)))))))

(defun ch-bag-tree-subbag? (tree1 tree2 hash-fn compare-fn)
  (declare (optimize (speed 3) (safety 0))
	   (type function hash-fn compare-fn))
  (rlabels (rec tree1 tree2 0)
    (rec (tree1 tree2 depth)
      ...
      (cond ((null tree1) t)
	    ...
	    ((consp tree1)
	     (if (consp tree2)
		 (wb-bag-tree-subbag? (cdr tree1) (cdr tree2) compare-fn)
	       (do-wb-bag-tree-pairs (x n (cdr tree1) t)
		 (unless (gen <= n (ch-bag-tree-multiplicity tree2 x hash-fn compare-fn depth))
		   (return nil)))))
	    ((consp tree2)
	     (do-ch-bag-tree-pairs (x n tree1 t)
	       (unless (gen <= n (wb-bag-tree-multiplicity (cdr tree2) x compare-fn))
		 (return nil))))
	    (t
	     (let ((entries1 (ch-bag-node-entry-mask tree1))
		   (entries2 (ch-bag-node-entry-mask tree2))
		 ...
		 (cond ((logbitp idx entries1)
			(cond ((logbitp idx entries2)
			       (let ((val1 count1 (ch-bag-node-entry tree1 (1-bits-below idx entries1)))
				     (val2 count2 (ch-bag-node-entry tree2 (1-bits-below idx entries2))))
				 (unless (and (equal?-cmp val1 val2 compare-fn)
					      (gen <= count1 count2))
				   (return nil))))
			      ((logbitp idx subnodes2)
			       (let ((val1 count1 (ch-bag-node-entry tree1 (1-bits-below idx entries1)))
				     (subnode2 (ch-bag-node-subnode tree2 (1-bits-below idx subnodes2))))
				 (unless (gen <= count1
					      (ch-bag-tree-multiplicity subnode2 val1 hash-fn compare-fn (1+ depth)))
				   (return nil))))
			      (t (return nil))))
		       ((logbitp idx subnodes1)
			(cond ((logbitp idx subnodes2)
			       (unless (rec (ch-bag-node-subnode tree1 (1-bits-below idx subnodes1))
					    (ch-bag-node-subnode tree2 (1-bits-below idx subnodes2))
					    (1+ depth))
				 (return nil)))
			      (t (return nil))))))))))))

(defun ch-bag-tree-disjoint? (tree1 tree2 hash-fn compare-fn)
  (declare (optimize (speed 3) (safety 0))
	   (type function hash-fn compare-fn))
  (rlabels (rec tree1 tree2 0)
    (rec (tree1 tree2 depth)
      ...
      (cond ((or (null tree1) (null tree2)) t)
	    ...
	    ((consp tree1)
	     (if (consp tree2)
		 (wb-bag-tree-disjoint? (cdr tree1) (cdr tree2) compare-fn)
	       (do-wb-bag-tree-pairs (x n (cdr tree1) t)
		 (declare (ignore n))
		 (when (/= 0 (ch-bag-tree-multiplicity tree2 x hash-fn compare-fn depth))
		   (return nil)))))
	    ((consp tree2)
	     (rec tree2 tree1 depth))
	    (t
	     (let ((entries1 (ch-bag-node-entry-mask tree1))
	       ...
	       (do-bit-indices (idx (logand (logior entries1 subnodes1) (logior entries2 subnodes2))
				    t)
		 (cond ((and (logbitp idx entries1) (logbitp idx entries2))
			(let ((val1 (ch-bag-node-entry tree1 (1-bits-below idx entries1)))
			      (val2 (ch-bag-node-entry tree2 (1-bits-below idx entries2))))
			  (when (equal?-cmp val1 val2 compare-fn)
			    (return nil))))
		       ((and (logbitp idx entries1) (logbitp idx subnodes2))
			(let ((val (ch-bag-node-entry tree1 (1-bits-below idx entries1))))
			  (when (/= 0 (ch-bag-tree-multiplicity (ch-bag-node-subnode tree2 (1-bits-below idx subnodes2))
								val hash-fn compare-fn (1+ depth)))
			    (return nil))))
		       ((and (logbitp idx subnodes1) (logbitp idx entries2))
			(let ((val (ch-bag-node-entry tree2 (1-bits-below idx entries2))))
			  (when (/= 0 (ch-bag-tree-multiplicity (ch-bag-node-subnode tree1 (1-bits-below idx subnodes1))
								val hash-fn compare-fn (1+ depth)))
			    (return nil))))
		       ((and (logbitp idx subnodes1) (logbitp idx subnodes2))
			(unless (rec (ch-bag-node-subnode tree1 (1-bits-below idx subnodes1))
...
(defun ch-set-tree-to-bag-tree (tree)
  (declare (optimize (speed 3) (safety 0))
	   (type ch-set-tree tree))
  (cond ((null tree) nil)
	((consp tree) (cons (car tree) (wb-set-tree-to-bag-tree (cdr tree))))
	(t
	 (let ((n-entries (logcount (ch-set-node-entry-mask tree)))
	       (n-subnodes (logcount (ch-set-node-subnode-mask tree)))
	       (tree-len (length tree))
	       ((len (+ ch-bag-node-header-size (* 2 n-entries) n-subnodes))
		((n (make-array len)))))
	   (setf (ch-bag-node-entry-mask n) (ch-set-node-entry-mask tree))
	   (setf (ch-bag-node-subnode-mask n) (ch-set-node-subnode-mask tree))
	   (setf (ch-bag-node-size n) (ch-set-node-size tree))
	   (setf (ch-bag-node-hash-value n) (ch-set-node-hash-value tree))
	   (setf (ch-bag-node-total-count n) (ch-set-node-size tree))
	   (dotimes (i n-entries)
	     (setf (svref n (+ ch-bag-node-header-size (* 2 i))) (svref tree (+ ch-set-node-header-size i)))
	     (setf (svref n (+ ch-bag-node-header-size (* 2 i) 1)) 1))
	   (dotimes (i n-subnodes)
	     (setf (svref n (- len i 1)) (ch-set-tree-to-bag-tree (svref tree (- tree-len i 1)))))
	   n))))

(defun ch-bag-tree-to-set-tree (tree)
  (declare (optimize (speed 3) (safety 0))
	   (type ch-bag-tree tree))
  (cond ((null tree) nil)
	((consp tree) (cons (car tree) (wb-bag-tree-to-set-tree (cdr tree))))
	(t
	 (let ((n-entries (logcount (ch-bag-node-entry-mask tree)))
	       (n-subnodes (logcount (ch-bag-node-subnode-mask tree)))
	       (tree-len (length tree))
	       ((len (+ ch-set-node-header-size n-entries n-subnodes))
		((n (make-array len)))))
	   (setf (ch-set-node-entry-mask n) (ch-bag-node-entry-mask tree))
	   (setf (ch-set-node-subnode-mask n) (ch-bag-node-subnode-mask tree))
	   (setf (ch-set-node-size n) (ch-bag-node-size tree))
	   (setf (ch-set-node-hash-value n) (ch-bag-node-hash-value tree))
	   (dotimes (i n-entries)
	     (setf (svref n (+ ch-set-node-header-size i)) (svref tree (+ ch-bag-node-header-size (* 2 i)))))
	   (dotimes (i n-subnodes)
	     (setf (svref n (- len i 1)) (ch-bag-tree-to-set-tree (svref tree (- tree-len i 1)))))
	   n))))
	  ...
	  (macrolet ((test (form)
	    ...
	    (let ((entry-mask (ch-bag-node-entry-mask node))
	      ...
	      (and (test (= 0 (logand entry-mask subnode-mask)))
		   ...
		   (test (= 0 (ash subnode-mask (- champ-node-radix))))
		   (test (>= (length node)
			     (+ ch-bag-node-header-size (* 2 (logcount entry-mask)) (logcount subnode-mask))))
		   ;; Check that unless root, this node does not contain only an entry ...
		   (test (not (and (> depth 0) (= 1 (logcount entry-mask)) (= 0 subnode-mask))))
		   ;; ... or only a collision subnode
		   (test (not (and (> depth 0) (= 0 entry-mask) (= 1 (logcount subnode-mask))
		   ...
		   (let ((size (logcount entry-mask))
		     ...
		     (and
		       ...
		       (gmap :and (fn (entry-idx hash-bits)
				    (let ((value (svref node (+ ch-bag-node-header-size (* 2 entry-idx))))
					  ((value-hash (hash-to-fixnum value hash-fn)))
					  (count (svref node (+ ch-bag-node-header-size (1+ (* 2 entry-idx))))))
		       ...
		       (gmap :and (fn (subnode-idx hash-bits)
					...
					(and (rec subnode (1+ depth) (new-partial-hash hash-bits depth partial-hash))
					     (progn
					       (incf size (ch-bag-node-size subnode))
					       (hash-mixf node-value-hash (ch-bag-node-hash-value subnode))
					       (gen incf node-total-count (ch-bag-node-total-count subnode))
					       t)))))
			     (:arg index 0)
			     (:arg list (bit-indices subnode-mask)))
		       ;; Finally, check size, hash, and total count
		       (test (= (ch-bag-node-size node) size))
		       (test (= (ch-bag-node-hash-value node) node-value-hash))
		       (test (gen = (ch-bag-node-total-count node) node-total-count))))))))
	(new-partial-hash (hash-bits depth partial-hash)
	  (dpb hash-bits (byte champ-hash-bits-per-level (* champ-hash-bits-per-level depth))
	       partial-hash))
	(path (depth partial-hash)
